"""
JournalTrace++ (standalone) — same USN Journal data as PathsParser++, sortable by
reason/timestamp/filename via the column headers (click to sort).
Run with no args for a GUI file picker, or: python3 journaltrace_tool.py usn_dump.bin out.csv
"""
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) > 2:
            with open(sys.argv[2], "w", newline="", encoding="utf-8") as f:
                w = csv.DictWriter(f, fieldnames=['usn', 'timestamp', 'reason', 'file_name', 'file_reference', 'parent_reference', 'file_attributes'])
                w.writeheader()
                w.writerows(rows)
    else:
        _run_gui('JournalTrace++ (standalone)', ['usn', 'timestamp', 'reason', 'file_name', 'file_reference', 'parent_reference', 'file_attributes'], _load_rows)